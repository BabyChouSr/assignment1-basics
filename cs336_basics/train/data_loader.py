...
def get_batch(x: np.array, batch_size: int, context_length: int, device: str):
    ...
    random_start_indices = torch.randint(0, possible_choices, (batch_size,), device=device)
    ...
    batch = torch.index_select(tensor, 0, indices.view(-1)).view(batch_size, context_length)
    target = torch.index_select(tensor, 0, (indices + 1).view(-1)).view(batch_size, context_length)
